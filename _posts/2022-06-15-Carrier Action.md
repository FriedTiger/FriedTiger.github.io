...
E(Total Electron Energy)는 편의상 E(electron potential Energy)를 선으로 그릴 수 있다. $E = E_p + E_k$​로 나타낼 수 있다. (점)= (벡터)+(선)


$$
E_{\text{potential E} \mid \text{electron}} = - q \cdot V_{\text{potential E}}
$$
따라서, V는 E를 뒤집은 형태이다. 
...
맥스웰 방정식에 의하면 다음과 같다. 
$$
\nabla \cdot \epsilon = \frac{\rho}{\epsilon_0}
$$
...
따라서 다음으로 표현된다.
$$
\sum_i\frac{v_i}{V}=Flux = J_{p\mid diff}=-qD_p\nabla p
\\J_{n\mid diff} = qD_n\nabla n
$$
로 표현가능하다.
